...
function buildVisionInstruction(
  ...
  if (mode === "polish") {
    return `
ТЫ ВЫПОЛНЯЕШЬ ПЕРЕВОД ТЕКСТА С ИЗОБРАЖЕНИЯ НА ПОЛЬСКИЙ ЯЗЫК.
СТРОГОЕ ПРАВИЛО:
Не описывай изображение.
Не объясняй, что изображено.
Не анализируй интерфейс телефона.
Не рассказывай о цветах, кнопках, статус-баре или дизайне.
Не пиши вступление.
Твоя задача:
1. Внимательно найди ВЕСЬ читаемый текст на изображении.
2. Распознай этот текст, включая текст в сообщениях, кнопках,
   уведомлениях, полях ввода и других элементах интерфейса.
3. Переведи распознанный текст на ЕСТЕСТВЕННЫЙ ПОЛЬСКИЙ язык.
4. Сохрани смысл и порядок текста.
5. Если на изображении несколько отдельных текстовых фрагментов,
   переведи их все.
6. Если часть текста невозможно прочитать, не выдумывай её.
7. В ответе дай ТОЛЬКО готовый польский перевод.
${userMessage
...
`.trim();
  ...
  if (mode === "english") {
    return `
ТЫ ВЫПОЛНЯЕШЬ ПЕРЕВОД ТЕКСТА С ИЗОБРАЖЕНИЯ НА АНГЛИЙСКИЙ ЯЗЫК.
СТРОГОЕ ПРАВИЛО:
Не описывай изображение.
Не объясняй, что изображено.
Не анализируй интерфейс телефона.
Не пиши вступление.
Твоя задача:
1. Найди весь читаемый текст на изображении.
2. Распознай его.
3. Переведи весь распознанный текст на естественный английский.
4. Сохрани смысл и порядок.
5. Не выдумывай нечитаемый текст.
6. В ответе дай только готовый перевод.
${userMessage
  ? `Дополнительное указание пользователя:\n${userMessage}`
  : ""}
`.trim();
  ...
  if (mode === "ukrainian") {
    return `
ТЫ ВЫПОЛНЯЕШЬ ПЕРЕВОД ТЕКСТА С ИЗОБРАЖЕНИЯ НА УКРАИНСКИЙ ЯЗЫК.
СТРОГОЕ ПРАВИЛО:
Не описывай изображение.
Не объясняй, что изображено.
Не анализируй интерфейс телефона.
...
1. Найди весь читаемый текст на изображении.
2. Распознай его.
3. Переведи весь распознанный текст на естественный украинский.
4. Сохрани смысл и порядок.
5. Не выдумывай нечитаемый текст.
6. В ответе дай только готовый перевод.
${userMessage
...
`.trim();
  ...
  return `
Проанализируй прикреплённые изображения и подробно опиши,
...
async function askOpenRouterVision(
  ...
  /*
   * ВАЖНО:
   * Для перевода с изображения инструкция
   * создаётся сервером, а не доверяется
   * только тексту, пришедшему от браузера.
   */
